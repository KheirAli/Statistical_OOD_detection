...
import numpy as np
import os
# os.environ['CUDA_VISIBLE_DEVICES'] = "7,5,4"
...
# guided-diffusion with learn_sigma=True returns [eps, var]
                if isinstance(et, tuple):
                    et = et[0]

                if et.shape[1] == 2 * x.shape[1]:
                    et = et[:, : x.shape[1], ...]
                elif et.shape[1] != x.shape[1]:
                    raise ValueError(
                        f"Unexpected UNet output channels: got {et.shape[1]}, "
                        f"expected {x.shape[1]} or {2 * x.shape[1]}"
                    )
