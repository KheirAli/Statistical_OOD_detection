...
def domain_adaptation(unet, config, fine_tune):
    ...
    # --- CHANGE START: dataset creation moved inside if fine_tune ---
    if fine_tune:      
        ...
        # Fix path joining for safe saving
        model_save_dir = os.path.join(os.path.join(os.getcwd(), config.model.checkpoint_dir), config.data.category)
        if not os.path.exists(model_save_dir):
            os.makedirs(model_save_dir, exist_ok=True)
            
        torch.save(frozen_feature_extractor.state_dict(), os.path.join(model_save_dir, f'feat0'))
        ...
        for epoch in range(config.model.DA_epochs):
            ...
            torch.save(feature_extractor.state_dict(), os.path.join(model_save_dir, f'feat{epoch+1}'))
    # --- CHANGE END ---
    else:
        if getattr(config.model, "use_frozen_fe", False):
...
# def domain_adaptation(unet, config, fine_tune):
    ...
    #     feature_extractor.load_state_dict(checkpoint)  
        ...
        if getattr(config.model, "use_frozen_fe", False):
            # use pretrained ImageNet backbone directly, no DDAD fine-tuning
            return feature_extractor
